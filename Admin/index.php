...
// bloc ajax : bloquer / debloquer un utilisateur (appele par le javascript du tableau)
if (isset($_GET["ajax"]) && $_GET["ajax"] === "blocage" && $_SERVER["REQUEST_METHOD"] === "POST") {
    header("Content-Type: application/json; charset=utf-8");
    // premiere securite : on verifie le role cote serveur (pas seulement dans l'affichage)
    if ($role_actuel !== "admin" && $role_actuel !== "fakeadmin") {
        http_response_code(403);
        echo json_encode(["ok" => false, "error" => "Accès refusé."], JSON_UNESCAPED_UNICODE);
        exit();
    }
    // deuxieme securite : on verifie le jeton csrf pour qu'un autre site ne puisse pas declencher l'action a notre place
    ...
    try {
        ...
        if ($target_id === $admin_id) {
            echo json_encode(
                ["ok" => false, "error" => "Vous ne pouvez pas bloquer votre propre compte."],
        ...
        if ((int) ($target["yumland"] ?? 0) !== 1) {
            echo json_encode(
                ["ok" => false, "error" => "Cet utilisateur ne fait pas partie de Yumland."],
                JSON_UNESCAPED_UNICODE,
            );
            exit();
        }
        ...
        if ($role_t === "admin" || $role_t === "fakeadmin") {
            echo json_encode(
                ["ok" => false, "error" => "Impossible de bloquer un compte administrateur."],
                JSON_UNESCAPED_UNICODE,
            );
            exit();
        }
        if ($action === "block") {
            ...
            try {
                ...
                $ins = $pdo_users->prepare("INSERT INTO blocages (user_id, bloque_par, motif) VALUES (?, ?, ?)");
...
// toute la partie admin (donnees + affichage) n'est accessible qu'aux admins : verifie cote serveur
if ($role_actuel === "admin" || $role_actuel === "fakeadmin") {
    // on charge les articles, surtout pour les compter dans les stats plus bas
    ...
    // on construit la requete de recherche par morceaux : on part d'une base, et on ajoute des bouts selon les filtres
    // les valeurs sont toujours liees en parametres (?) pour eviter l'injection sql
    $sql = "SELECT * FROM users WHERE yumland = ?";
    $exec = [$yumland];

    // si on a tape quelque chose dans la barre de recherche, on ajoute le morceau correspondant
    ...
    // si on a choisi un role dans le filtre, on ajoute le morceau qui va bien
    if (isset($_GET["role"])) {
        $role = $_GET["role"];
        switch ($role) {
            case "admin":
    ...
    //on dégage l'utilisateur qui n'est pas admin ou fakeadmin
...
{
    //On passe la date en français pour l'affichage puisque dans la database elle est en américain
...
<head>
  ...
  <script>
    // ce script gere les boutons bloquer / debloquer de la liste des utilisateurs, en ajax
    // place dans le head : on attend le chargement de la page avant d'agir sur les elements
    document.addEventListener('DOMContentLoaded', function () {
        // on recupere le jeton csrf depose dans une balise meta : il protege les actions de blocage
        const csrfMeta = document.querySelector('meta[name="csrf-token"]');
        const csrfToken = csrfMeta ? csrfMeta.getAttribute('content') || '' : '';
        const feedback = document.getElementById('admin-blocage-feedback');

        // affiche un petit message sous le tableau (rouge si c'est une erreur)
        ...
        // remplace le bouton bloquer par debloquer (ou l'inverse) apres une action reussie
        function swapBlockButton(tr, nowBlocked) {
            const cell = tr.querySelector('.admin-user-row-actions');
            if (!cell) return;
            const vip = cell.querySelector('.admin-fake-action-btn--vip');
            cell.innerHTML = '';
            if (nowBlocked) {
        ...
        // branche le bouton bloquer/debloquer d'une ligne du tableau
        function bindOneRow(tr) {
            const btn = tr.querySelector('.admin-blocage-btn');
            if (!btn) return;
            btn.addEventListener('click', function () {
                const userId = parseInt(tr.getAttribute('data-user-id') || '0', 10);
                const action = btn.getAttribute('data-action') || '';
                let motif = null;
                // pour un blocage on demande un motif optionnel ; si on annule la fenetre, on annule l'action
                if (action === 'block') {
                ...
                btn.disabled = true;
                // on envoie l'action au serveur en ajax, avec le jeton csrf dans l'entete
                fetch('index.php?ajax=blocage', {
                    ...
                    .then(function (data) {
                        ...
                        // on met a jour l'affichage de la ligne selon le nouvel etat (bloque ou non)
        ...
        // au chargement, on branche toutes les lignes utilisateurs du tableau
...
<body>
    <div class="pcnavbar">
        ...
        <div class="navbar-inner">
            ...
            <?php endif; ?>
            <?php if ($est_connecte && ($role_actuel === "admin" || $role_actuel === "fakeadmin")): ?>
                <li><a href="<?= $BASE ?>/Admin/additems.php">Ajouter un article</a></li>
            <?php endif; ?>
            </ul>
    ...
    <div id="mainMenuContainer">
        ...
        <?php if (!$est_connecte): ?>
            ...
            <?php if ($est_connecte && ($role_actuel === "admin" || $role_actuel === "fakeadmin")): ?>
            <div class="mainMenuItemLogin">
                <a href="<?= $BASE ?>/Admin/additems.php">
                <span class="mainMenuItemCollapsable">
                    <img src="../images/accueil.png" alt="Menu item">
                </span>
    ...
    <form method="GET" action="index.php">
        <section class="Filtres">
            ...
            <!-- ce filtre est inactif pour l'instant : il n'a pas encore de traitement cote serveur -->
            <div class="filtres">
                <label for="filtre_statut">Filtrer (inactif) :</label>
            ...
            <!-- ce tri est inactif pour l'instant : il n'a pas encore de traitement cote serveur -->
            <div class="trier">
                <label for="tri">Trier (inactif) :</label>
    ...
    <section class="table">
        <table>
            ...
            <?php foreach ($tableau_users_page as $user): ?>
                ...
                $is_protected_row =
                    ($current_admin_user_id !== null && $uid_row === $current_admin_user_id) ||
                    $role_row === "admin" ||
                    $role_row === "fakeadmin";
                ?>
                <tr class="admin-user-row<?php echo $is_blocked_row
                    ? " admin-user-row--blocked"
                    : ""; ?>" data-user-id="<?php echo $uid_row; ?>">
