...
<?php
  ...
  $message = null;
    ...
    // ── Catalogue (même source que Carte/index.php) 
    $catalog = [];
    foreach (json_decode(file_get_contents('../data/items.json'), true) as $items) {
            ...
            if ($db_user_id === null) {
                ...
                try {
                    ...
                    $stmt = $pdo_commandes->prepare(
                        "INSERT INTO commandes (user_id, email, date_commande, total, nb_articles, statut, heure_livraison) VALUES (?, ?, ?, ?, ?, ?, ?)"
                    );
                    $stmt->execute([$db_user_id, $db_user_email, $date_commande, $panier_total, $panier_count, $statut, $heure_livraison]);
