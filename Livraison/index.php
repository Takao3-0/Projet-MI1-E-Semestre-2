...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["accept_course"])) {
    ...
    if ($cid > 0) {
        $sql =
            ...
            LIVREUR_ID_COL .
            ' = ?, statut_production = \'Récupérée livreur\'
                 WHERE id = ? AND statut = \'Payé\' AND statut_production = \'Prête\'
                   AND ' .
            LIVRAISON_SQL_UNASSIGNED;
...
// affiche le bloc des gains (frais de livraison moins frais de gestion).
// appele dans les deux listes de cartes : comme ca le html des gains n'existe qu'a un seul endroit
function livraison_afficher_gains(float $frais_liv): void
{
...
<body class="livraison-page">
    ...
    <section class="livraison-pool-sticky" aria-label="Commandes prêtes sans livreur">
        ...
        <?php foreach ($disponibles as $cmd): ?>
            ...
            $heure = isset($cmd["heure_livraison"])
                ? htmlspecialchars((string) $cmd["heure_livraison"], ENT_QUOTES, "UTF-8")
                : "";
            $date_cmd = isset($cmd["date_commande"])
                ? htmlspecialchars((string) $cmd["date_commande"], ENT_QUOTES, "UTF-8")
                : "";
            ...
            <article class="livraison-card livraison-card--pool">
                ...
                <div class="livraison-client livraison-client--minimal">
                    ...
                    <?php livraison_afficher_gains($frais_liv); ?>
                ...
                <div class="livraison-card-footer">
                    <form method="post" action="index.php" class="livraison-accept-form">
                        ...
                        <button type="submit" class="btn-accept-course" <?php echo $livreur_warn_no_uid
                            ? "disabled"
                            : ""; ?>>
