...
$pdo_commandes = $pdo;
// ce fichier contient la fonction statut_slug dont on se sert plus bas pour la couleur du statut
require_once __DIR__ . "/fonctions.php";

// on prend toutes les commandes payees, et plus bas on gardera seulement celles du compte connecte
$stmt = $pdo_commandes->prepare("SELECT * FROM commandes WHERE statut = 'Payé'");
$stmt->execute();
$commandes_order_new = $stmt->fetchAll();

// on retrouve le compte connecte pour pouvoir comparer son email avec celui des commandes
...
<head>
    ...
    <style>
        /* ces styles servent seulement pour la liste de commandes de cette page */
        ...
        @media (max-width: 600px) {
            ...
            .historique > div > .commande-link .prix {
                align-self: flex-end;
            }
            .historique .commande-chevron {
                display: none;
...
<body>
    ...
    <?php // ce drapeau nous dira a la fin si on a affiche au moins une commande
            // on passe sur toutes les commandes payees recuperees plus haut
            // on n'affiche la commande que si son email est celui du compte connecte
            // statut_slug transforme le statut en nom de classe css pour avoir la bonne couleur
            // fin du test sur l'email
...
<div class="historique">
    ...
    <div>
                <?php
            // ce drapeau nous dira a la fin si on a affiche au moins une commande
            ?>
            ...
            ?>
                <?php foreach ($commandes_order_new as $commande): ?>
                    ...
                    <?php if ($user_actuel["email"] == $commande["email"]): ?>
                            ...
                            // statut_slug transforme le statut en nom de classe css pour avoir la bonne couleur
                            ?>
                                                        <span class="commande-statut commande-statut-<?= htmlspecialchars(
                                                                statut_slug($commande["statut_production"]),
                                                        ) ?>">
                    ...
                    <?php endif;endforeach;endif; // fin de la boucle sur les commandes // ferme le else du test est connecte du haut de la page
    // si commande_trouve est reste a false, c'est que le compte n'a passe aucune commande
    if (!$commande_trouve): ?>
