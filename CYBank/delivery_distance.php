...
// ce fichier calcule les frais de livraison a partir de l'adresse du client
// il demande l'itineraire a google maps (distance et duree), puis applique une grille de tarifs
...
const DELIVERY_PRICING = [
    "base_fee" => 1.5, // frais fixes (€)
    "per_km" => 0.8, // coût par km (€)
    "min_fee" => 2.5, // minimum de frais de livraison (€)
    "max_distance_km" => 20.0, // distance max acceptée (km)
    "free_above" => 0.0, // livraison gratuite au-dessus de X€ de commande
    "fallback_fee" => 2.5, // forfait fixe quand on ignore le calcul d'itinéraire (API Google indispo)
];

// fonction principale : prend une adresse, renvoie le cout de livraison (ou une erreur) dans un tableau
function deliveryEstimate(string $clientAddress, ?float $orderTotal = null): array
{
    ...
    if ($maxKm > 0 && $distanceKm > $maxKm) {
        return deliveryError("Adresse trop éloignée ({$distanceKm} km). Livraison possible jusqu'à {$maxKm} km.", [
